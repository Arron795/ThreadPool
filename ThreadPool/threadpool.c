...
struct ThreadPool
{
	...
	int minNum;					//最小线程数
	int maxNum;					//最大线程数
	int busyNum;				//忙的线程的个数 （当前工作线程的个数）
	int liveNum;				//存活的线程个数  （当前存活，包括工作的和空闲的线程个数）
...
ThreadPool* threadPoolCreate(int min, int max, int queueSize)
{
	...
	do
	{
		...
		{
			printf("mutex and condition init fail... \n");
			break;
		}
	...
	{
		free(pool && pool->threadIDs);
	}
	if (pool && pool->taskQ)
	{
		free(pool && pool->taskQ);
	}
	if (pool)
	{
		free(pool);
...
void* worker(void* arg)
{
	...
	while (1)
	{
		pthread_mutex_lock(&pool->mutexPool);
		//当前任务队列是否为空
		while (pool->queueSize == 0 && !pool->shutdown)		//任务数量为0 且线程池没有被关闭
		{
			//阻塞工作线程
